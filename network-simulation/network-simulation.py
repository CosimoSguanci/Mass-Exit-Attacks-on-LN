...
ACCUMULATED_TXS = 0
MINER_ADDRESS = ""
RESULTS_DIR = f"{ENVIRONMENT_HOME}/Desktop/results"
...
WATCHTOWER_DIR = f"{ROOT_DIR}/.lnd-watchtower"
...
LND3_COMMAND = f"{LN_EXE_DIR}/lnd --lnddir={WATCHTOWER_DIR} --accept-keysend"
...
LNCLI3_COMMAND = f"{LN_EXE_DIR}/lncli -n regtest --lnddir={WATCHTOWER_DIR} --rpcserver=localhost:11010"
...
def setup_logs():
    ...
    logFile = logging.FileHandler(f"{DEV_DIR}/network-simulation/simulation.log")
...
def clean_logs():
    if os.path.exists(f"{DEV_DIR}/network-simulation/simulation.log"):
        os.remove(f"{DEV_DIR}/network-simulation/simulation.log")
...
def setup_lnd_nodes():
    ...
    with open(f'{WATCHTOWER_DIR}/lnd.conf', "w+") as lnd_conf:
        lnd_conf.write(LND_CONF_WATCHTOWER)
...
def parse_watchtower_logs():
    justice_txids = []
    logFile = open(f'{WATCHTOWER_DIR}/logs/bitcoin/regtest/lnd.log', 'r') 
    lines = logFile.readlines()
    compromised_channels = 0

    for log in lines:
        if "Publishing justice transaction for client" in log:
            justice_txid = log[log.index("txid=") + 5: len(log) - 1]
            justice_txids.append(justice_txid)

            if len(justice_txids) == N_ATTACKED_CHANNELS:
                break
        
    for justice_txid in justice_txids:
        try:
            command = f"{BITCOIN_CLI_PATH}/bitcoin-cli getrawtransaction {justice_txid} 1"
            output = check_output(shlex.split(command), stderr=STDOUT).decode()
            output = json.loads(output)
            if "blockhash" in output:
                justice_block_hash = output["blockhash"]
                justice_tx_fee = compute_tx_fee_from_raw_tx(output)
                command = f"{BITCOIN_CLI_PATH}/bitcoin-cli getblock {justice_block_hash}"
                output = check_output(shlex.split(
                    command), stderr=STDOUT).decode()
                output = json.loads(output)
                justice_block_number = output["height"]
                LOGGER.info(f"Justice transaction with id={justice_txid} confirmed in block {justice_block_number} with fee rate of {justice_tx_fee} sat/vbyte")
            else:
                LOGGER.info(f"Justice transaction with id={justice_txid} with fee rate of {justice_tx_fee} sat/vbyte could still be unconfirmed but in the mempool...")
        except:
            # No such mempool or blockchain transaction
            LOGGER.info(f"Justice transaction with id={justice_txid} could not be found in the mempool, COMPROMISED CHANNEL")
            compromised_channels += 1
    
    LOGGER.info(f"{compromised_channels} compromised channels")
...
def log_walletbalance():
    ...
    command = f"{LNCLI1_COMMAND} walletbalance"
    output = check_output(shlex.split(command), stderr=STDOUT).decode()
    output = json.loads(output)
...
def check_breaches(current_block_number):
    ...
    if len(CONFIRMED_BREACHES) < N_ATTACKED_CHANNELS:
        ...
        if len(BREACH_TXIDS) < N_ATTACKED_CHANNELS:
            ...
            for log in lines:
                if "Dispatching punisher for client" in log:
                    breach_txid = log[log.index("breach-txid=") + 12: len(log) - 1]
...
def check_end_condition(current_block_number):
    ...
    if len(JUSTICE_TXIDS) < N_ATTACKED_CHANNELS:
        ...
        for log in lines:
            if "Publishing justice transaction for client" in log:
                justice_txid = log[log.index("txid=") + 5: len(log) - 1]
                if not justice_txid in JUSTICE_TXIDS:
                    JUSTICE_TXIDS.append(justice_txid)
    ...
    for justice_txid in JUSTICE_TXIDS:
        if not justice_txid in CONFIRMED_JUSTICE:
            try:
                command = f"{BITCOIN_CLI_PATH}/bitcoin-cli getrawtransaction {justice_txid} 1"
                output = check_output(shlex.split(command), stderr=STDOUT).decode()
                output = json.loads(output)
                if "blockhash" in output:
                    ...
                    if not justice_txid in CONFIRMED_JUSTICE:
                        CONFIRMED_JUSTICE[justice_txid] = True
                        LOGGER.info(f"Justice transaction with txid={justice_txid} confirmed after {current_block_number} blocks of simulation")
            except:
                # No such mempool or blockchain transaction
                finalized_channels += 1
        else:
            finalized_channels += 1
    if finalized_channels > 0:
        LOGGER.info(f"{finalized_channels} finalized channels")

    if finalized_channels == N_ATTACKED_CHANNELS:
        command = f"{LNCLI2_COMMAND} walletbalance"
        output = check_output(shlex.split(command), stderr=STDOUT).decode()
        output = json.loads(output)
        unconfirmed_balance = int(output["unconfirmed_balance"])
        while unconfirmed_balance >= CHANNEL_CAPACITY:
            generate_blocks(20)
            command = f"{LNCLI2_COMMAND} walletbalance"
            output = check_output(shlex.split(command), stderr=STDOUT).decode()
            output = json.loads(output)
    ...
    for snapshot in mempool_data:
        ...
        if total_tx_count < last_total_tx_count and not is_in_problematic_interval:
            ...
            for i in range(blocksCount):
                ...
                if check_end_condition(current_block_number):
...
def generate_block_from_data(blocks, blocksCounter, firstBlockHeightOfSimulation):
    ...
    list_sweeps = get_attacker_list_sweeps()
        ...
        try:
            ...
            while block_tx_count < int(num_tx):
                ...
                if next_txid in list_sweeps:
                    ...
                    for txinput in vin:
                        if txinput["txid"] in PENDING_SWEEPS_COMMITMENT_ANCHOR and not txinput["txid"] in CONFIRMED_BREACHES:
...
def do_initial_operations():
    ...
    command = f"{LNCLI3_COMMAND} tower info"
    output = check_output(shlex.split(command), stderr=STDOUT).decode()
    output = json.loads(output)
    tower_pubkey = output["pubkey"]

    command = f"{LNCLI1_COMMAND} wtclient add {tower_pubkey}@127.0.0.1:9911"
    output = check_output(shlex.split(command), stderr=STDOUT).decode()
    output = json.loads(output)
    ...
    command = f"{LNCLI3_COMMAND} newaddress np2wkh"
    output = check_output(shlex.split(command), stderr=STDOUT).decode()
    output = json.loads(output)
    watchtower_address = output["address"]
    ...
    command = f"{BITCOIN_CLI_PATH}/bitcoin-cli sendtoaddress {watchtower_address} 500"
    output = check_output(shlex.split(command), stderr=STDOUT).decode()
...
def create_and_start_lnd_instances():
    ...
    LOGGER.info("Starting watchtower's LND")
    
    command = "systemctl --user enable --now lnd3.service"
    output = check_output(shlex.split(command), stderr=STDOUT).decode()
    print(output)
    time.sleep(1)

    child = pexpect.spawn(f'{LNCLI3_COMMAND} create')
    time.sleep(1)
    child.sendline('password')
    child.sendline('password')
    child.sendline('n')
...
def clean_processes():
    ...
    command = "systemctl --user disable --now lnd3.service"
    check_output(shlex.split(command), stderr=STDOUT).decode()
...
def sim():
    clean_logs()
    setup_logs()
    LOGGER.info(f"STARTING SIMULATION WITH BREACH TX FEE RATE = {BREACH_TX_FEE_RATE}, TO_SELF_DELAY = {TO_SELF_DELAY}")
    clean_processes()
    clean_dirs()
    setup_lnd_nodes()
    start_bitcoind()
    ...
    if not ret_val:
        return False
    # Finally, we parse the log from the watchtower to check how many channels have been "breached"
    parse_watchtower_logs()
    log_walletbalance()
...
def main():
    ...
    for experiment in experiments:
        ...
        shutil.copyfile(f"{DEV_DIR}/network-simulation/simulation.log", f"{res_dir}/simulation.log")
